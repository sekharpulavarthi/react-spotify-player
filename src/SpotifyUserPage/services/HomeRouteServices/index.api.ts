...
export class HomeRouteServiceAPI implements HomeServiceType {
  editorsPicksServiceAPI = async (): Promise<HomeRouteFetchedResponseTypes> => {
    ...
    const response = await fetch(
      `https://api.spotify.com/v1/browse/featured-playlists?country=IN&timestamp=${timeStamp}`,
      options
    );
  ...
  newReleasesServiceAPI = async (): Promise<
    HomeRouteFetchedNewReleasesResponseTypes
  > => {
    const options = {
      method: "GET",
  ...
  genresMoodsServiceAPI = async (): Promise<
    HomeRouteGenresMoodsResponseFetchedTypes
  > => {
    const options = {
      method: "GET",
